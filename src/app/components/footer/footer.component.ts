    ...
    <footer class="foot">
      <div class="contenedor foot-grid">
        <div>
          ...
          <p>Fresas con crema y mÃ¡s delicias. Dulces momentos, hechos con amor.</p>
        </div>
        <nav aria-label="Enlaces de pie de pÃ¡gina">
          <a routerLink="/" fragment="catalogo">MenÃº</a>
          <a routerLink="/checkout">Carrito</a>
          <a routerLink="/auth/login">Clientes</a>
          <a routerLink="/admin/dashboard">Admin</a>
        </nav>
        <div class="legal">
          <strong>Â© {{ year }} Antojitos S&amp;A</strong>
          <span>Todos los antojos reservados.</span>
        </div>
      </div>
    </footer>
  `,
