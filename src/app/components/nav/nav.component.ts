...
@Component({
    ...
    <header class="nav">
      <div class="contenedor nav-row">
        ...
        <div class="acciones">
          ...
          @if (auth.logueado()) { <a routerLink="/cuenta/perfil" class="usuario">{{ saludo() }}</a> } @else { <a routerLink="/auth/login" class="btn-fantasma compact">Entrar</a><a routerLink="/auth/registro" class="btn-primario compact">Cuenta</a> }
        </div>
      </div>
    </header>
  `,
  styles: [`
    ...
    @media (max-width: 640px) { .brand span { font-size: 18px; } .acciones .btn-primario, .acciones .btn-fantasma { display: none; } }
  `]
})
export class NavComponent {
  carrito = inject(CarritoService);
  auth = inject(AuthService);
  saludo(): string { const nombre = this.auth.perfil()?.nombre; return nombre ? nombre.split(' ')[0] : 'Mi cuenta'; }
}
